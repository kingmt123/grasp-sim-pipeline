...
| 指标 | 结果 | 复现命令 |
|---|---|---|
| **抓取成功率（严格判据，固定配置 n=10）** | **30/30 (100%)** | `--grasp-trials 10` |
| **cube 面对面呈现（yaw≈0°）** | **10/10 (100%)**，夹持开度 35.6mm | `--objects cube --grasp-trials 10` |
| **cube 角对角呈现（yaw≈30/45°）** | **0/10 (0%)** — 空夹，夹持开度 0.0mm | `--objects cube --grasp-trials 10 --rot 45` |
| 抓取成功率（±2cm 位置抖动 n=10） | 29/30（**旧判据**，严格判据待复测） | `--grasp-trials 10 --jitter 0.02` |
...
> **判据口径（重要）**：成功 = ①轻提 Δz>1cm **且** ②抬到高位保持 240 仿真步不滑落 **且** ③夹爪开度>5mm（两指间确实有物体）
> **且** ④保持结束时不再接触支撑面。旧判据只有 ①，会把"被顶起后掉回"与"空夹"算成成功
> （实测：cube 角对角时旧判据报成功、严格判据 0/10）。设 `GRASP_LEGACY_CRITERION=1` 可回退旧判据做对比。
> 以上均为**仿真内**结果，**无真机验证**；定位精度以 GT **AABB 中点**为参照。
>
...
**6D 姿态（偏航）—— 结论已订正：偏航是真实需求**（完整数据见 `log.md` STEP13/STEP14）

- 融合点云 **PCA** 给不出可用偏航：与 GT 误差中位 teddy 15.7°、duck 70.2°、cube 不可观测（正方形足迹 → 2D 协方差退化）。
- 但**升级后的严格判据**揭示：**cube 以角对角呈现（yaw 偏离面对面 ≈30–45°）时抓取 0/10（空夹 0.0mm）**，
  面对面呈现则 10/10。⇒ **偏航与夹爪轴的错位超过约 20° 就会失败，"±45° 零代价"的旧结论作废**
  （那是弱判据 + cube 恰好面对面的巧合）。
- 待办（已排期）：用点云足迹的 **minAreaRect 角度**作为 yaw（PCA 对方块退化，最小面积矩形对它有定义），
  在 0/30/45° 三档验证，目标把角对角从 0/10 提到 ≥9/10。
- 其它触发条件不变：出现**细长/扁平物体**、需要**定向放置/插入**、或**换真机**（无 GT oracle）时，
  再评估 CAD 模板匹配 / 学习式 6D（FoundationPose 类）。
- 诊断/测试工具：`scripts/diagnose_yaw.py`、`batch_test_final.py --rot N`。
