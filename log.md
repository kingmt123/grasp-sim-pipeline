...
> ⚠️ **2026-09-10 订正（见 STEP12）**：不存在"内参系统偏差"。真主点就是 (320, 240)，真焦距 415.69px。当年需要的 -88 / -8 / -54 是在补"焦距用错 FOV 轴（偏大 4/3）"这个模型错误。
...
> ⚠️ **2026-09-10 订正（见 STEP12）**：现象记录正确，归因错误。不是"cy_offset 是图像空间参数"，而是焦距偏大 4/3 —— 一个错误的焦距只能被"某个像素区间内的 cy 偏移"近似抵消，所以每个相机朝向看起来都需要不同的 offset。
...
| ~~IK 收敛极限 ~3.9cm~~ **已订正（STEP12）** | ~~P0~~ 假象 | 读的是 `getLinkState()[0]`=CoM，而 IK 控制的是 link frame `[4]`，两者恒差 3.90cm | 真实 IK 残差 ≤0.1cm，无需任何补偿 |
...
| ~~固定 cy_offset 非最优~~ **已订正（STEP12）** | ~~P2~~ 已消除 | 内参修正后 `cy_offset = 0`（真主点 240） | — |
| 固定桌面高度公式硬编码 | **P2** | 物体高度值和比率需手动设定 | 新增物体需改代码 |
| ~~固定桌面高度公式硬编码~~ **已订正（STEP12）** | ~~P2~~ 已解决 | 改为"夹持面覆盖物体"物理规则（实测夹持面 0.062m） | 新增物体无需改代码 |
...
## STEP12: Z 系统性偏高根因（内参模型 + 末端坐标系）+ 全面复验

**日期**: 2026-09-10 | **分支**: `fix/intrinsics-z-bias` (`7f77432`) → 合并 `37ce12d`

### 结论一句话

Z 偏高不是标定问题，也不是"内参系统偏差"——是**两个内参模型 bug + 两个末端坐标系 bug**。
`cy_offset`（-88 → -8 → -54）从头到尾都是 bug#1 的凑合量。

### 根因（4 项，每项都有可复现的实测证据）

| # | Bug | 证据（决定性判据） | 影响 |
|---|-----|------------------|------|
| 1 | **焦距用错 FOV 轴**：`computeProjectionMatrixFOV` 的 `fov` 是**垂直** FOV，像素焦距 `= (H/2)/tan(fov/2) = 415.69px`；代码用了 `(W/2)/tan` → **偏大 4/3 倍** | 重投影残差 `\|Δu\|` 逐像素等于 `0.25×\|u−320\|`；换成 `(H/2)` 后 `\|Δu\| = 0.00px`。直接解析投影矩阵：`m00=1.299, m02=0, m11=1.732, m12=0` → `fx=fy=415.69, cx=320, cy=240` | 射线整体压缩，离图像中心越远越偏；必须靠一个"随相机朝向变化"的 `cy_offset` 抵消 → STEP4 的"cy_offset 不能跨视角共用"现象 |
| 2 | **反投影 y 轴符号反了**：`y_cam = +(v−cy)·z/fy`，正确为 `−` | v 方向残差 `\|Δv\| ≈ 2\|v−240\|`（镜像特征，u 方向为 0）；加负号后 `\|Δu\|=\|Δv\|=0.00px`；rayTest 命中率 **4.6% → 95.7%**，命中样本 3D 误差中位 **0.18cm** | 竖直方向镜像 → 融合质心 Z 恒定偏高 |
| 3 | **末端读错坐标系**：`getLinkState()[0]` 是**质心 CoM**，IK 真正控制的是 **link frame `[4]`**，两者恒差 **3.90cm** | 目标 0.750/0.800/0.900 三档，`[0]` 残差恒为 +3.90cm，`[4]` 残差 ≤0.10cm | 伪造出"IK 工作空间极限 3.9cm"；据此设的 `ik_bias` 是补偿量；`ik_controller` 收敛判据永不成立（每次白跑满 600/800 步） |
| 4 | **`FINGER_OFFSET` 与真实指尖几何不符**：旧值 0.105 实为 link frame→`panda_grasptarget` 距离 | 实测 link frame → 手指碰撞 AABB 最低点 = **0.1162m**（`p.getAABB(robot, linkIndex=9/10)`） | "指尖高度"与安全钳制的物理含义整体错位 |

### 为什么潜伏了 5 个月：两个错误在特定条件下互相抵消

1. `f` 偏大 4/3 使射线压缩 —— 而"平移 `cy`"恰好能在**某个像素区间**内近似补回。
2. 于是每个相机朝向都"需要"不同的 offset（STEP4 记录为"cy_offset 是图像空间参数"）。
3. STEP8 把相机改成**迭代指向物体**（物体被摆到图像中心）后，补偿条件失效，剩下的 54px 偏差被原样注入 → 表现为**恒定 +8~12cm Z 偏差**。
4. XY 受害小：u 方向只有 f 误差，且误差 `∝ (u−cx)`，而 Stage2 让物体居中 → XY 一直是 1.5cm。

### 修复清单

- `src/perception/pose_estimator.py`：`fx=fy=(CAM_HEIGHT/2)/tan(fov/2)`；`y_cam` 取负；`cy_offset` 默认 0
- `src/control/ik_controller.py`：收敛判据改用 link frame `[4]`（此前永不收敛）
- `scripts/test_multi_object_grasp.py`：`FIXED_CY=0`；`FINGER_OFFSET=0.1162`；`get_ee_pos` 用 `[4]`；`ik_bias` 归零（仅留 mm 级微调位）；fine 阶段加**实测残差闭环重发目标**；抓取高度改为**"夹持面覆盖物体"物理规则**（删除 per-object `waist_ratio` 凑合表）
- `scripts/batch_test_final.py`：定位评分基准改为 **GT AABB 中点**（可见表面质心的正确参照，此前拿 URDF 基座比 → 天然差半个物体高度）
- 新增工具：`diagnose_z_bias.py`（内参四检验）、`diagnose_finger_geometry.py`（末端几何实测）、`sweep_grasp_height.py`（抓取高度扫描）

### 复验（同机、同 harness、同日前后对比）

| 指标 | 修正前 | 修正后 |
|------|--------|--------|
| 定位 XY (duck/teddy/cube) | 1.47 / 1.39 / 1.46 cm | **0.75 / 1.37 / 0.92 cm** |
| 定位 Z vs GT AABB 中点 | +5.2 / +6.2 / +7.2 cm | **+0.12 / +0.73 / +0.82 cm** |
| 定位 Z vs URDF 基座（旧口径） | +11.7 / +8.2 / +9.0 cm | +2.5 / +0.7 / +0.8 cm |
| **逐视角 Z 一致性** | 5.0–7.0 cm 分歧 | **0.3–0.4 cm** |
| rayTest（GT mask 像素） | 命中 4.6% | **命中 95.7%，3D 误差中位 0.18cm** |
| 抓取 固定配置 3 轮 | 9/9 | **9/9** |
| 抓取 ±2cm 位置抖动 3 轮 | 7/9（duck **1/3**） | **8/9（duck 3/3、cube 3/3）** |
| 抓取 固定配置 **n=10**（2026-09-12） | — | **30/30 (100%)** |
| 抓取 ±2cm 抖动 **n=10**（2026-09-12） | — | **29/30 (97%)**：duck 10/10、teddy **9/10**、cube 10/10 |
| IK 残差 | 名义 3.9cm | **≤2mm** |
| 自由度 | `cy_offset`+`waist_ratio`表+`ik_bias` 三个凑合量 | 全部消除，只剩物理量 |

> 注：固定配置下 σ=0（管线完全确定性），**N 轮只证明可重复性，不证明鲁棒性**；鲁棒性看抖动那一行。

### 被推翻的旧结论（已在原处加订正标注）

1. ~~"IK 工作空间边缘 ~3.9cm 收敛极限"（STEP7/8 的 P0）~~ → 坐标系误读，真实残差 ≤0.1cm。
2. ~~"IK 精细阶段始终 ~3.9cm"~~ → 同上，控制器现在会真正收敛并提前退出。
3. ~~"cube 抓取成功 ~66%"~~ → 旧成功是**贴桌支助**（2 个接触点、穿透 −0.010、物理指尖在桌面以下）；现在是真实夹持（5 接触点、Δz 4.9cm）。
4. ~~"cy_offset 需按物体/按视角标定"、"内参有系统偏差"~~ → 假象，见 bug#1。
5. teddy "焦外退化 → mask 丢失 → Y 偏差 5-8cm" 定为 P0：内参修正后 teddy XY 误差 1.37cm（DIRECT+GT mask 下 0.42cm），已不再是主要矛盾。

### 遗留（诚实留白）

- **抖动下 teddy 9/10（n=10，2026-09-12）**：60 次抓取中唯一失败是 teddy 一次 Δz=0.8cm（判据 1cm），
  其余 59/60 成功（固定配置 30/30）。软点范围已限定为"位置抖动 + teddy 临界夹持"，
  不是系统性缺陷；若要继续压，方向是 teddy 的夹持高度/力度而非定位。
- 6D 姿态、惯性张量偏航仍未做；`CLAUDE.md` 的对应参数描述待订正（写保护文件）。
- YOLO 训练/推理渲染器不一致（数据集 `ER_BULLET_HARDWARE_OPENGL` vs `get_camera_image` 默认 `ER_TINY_RENDERER`）未处理。

### 证据与工具入口

```bash
uv run python scripts/diagnose_z_bias.py            # 根因：内参四检验（投影矩阵/重投影/rayTest/平面）
uv run python scripts/diagnose_finger_geometry.py   # 末端真实几何（CoM vs link frame、指尖、夹持面）
uv run python scripts/sweep_grasp_height.py --heights 0.60,0.62,0.64,0.66   # 抓取高度扫描
uv run python scripts/batch_test_final.py --pose-trials 5 --grasp-trials 3 [--jitter 0.02]  # 三级复验
```

- 明细: `runs/z_bias_diag_20260910_214848.json`、`runs/batch_stats_20260910_223432.json`（固定 9/9）、`runs/batch_stats_20260910_223703.json`（抖动 8/9）
- 运行日志: `D:\HermesWorkspace\{z_diag4.log, sweep_height.log, verify_fix3.log}`

---

## STEP13: 6D 姿态（偏航）预研 —— 测量结果与待决策项

**日期**: 2026-09-12 | **状态**: 预研完成，**决策待定**（未改主线接口）

### 问题

主线偏航是 per-object 硬编码（duck 0° / teddy 90° / cube 0°，见 `test_multi_object_grasp.py`），
理由是 STEP7 的结论"PCA 在单视角/多视角均不可行"。但该结论是在内参被修正前得出的，
需要重新验证：**修完内参后，偏航是否已经可以从融合点云可靠估计？**

### 测量一：融合点云 PCA 的偏航可观测性（`scripts/diagnose_yaw.py`）

把物体绕世界 Z 逐档旋转 0/30/60/90/135°，比较点云建议偏航的**追踪性**（每档真实转 +30°）：

| 物体 | 点云建议偏航（5 档） | 相邻档变化 | 与 GT 误差中位 | 伸长比 | 判定 |
|------|--------------------|-----------|---------------|--------|------|
| teddy | 167→202→247→−82→−48° | +35 / +45 / +30 / +34 | **15.7°** | 1.15–1.54 | 弱可用（噪声 5–25°） |
| duck | −70→−50→156→159→173° | +19.7 / +26.0 / **+3.1** / +14.2 | **70.2°** | 1.15–1.45 | 不可用 |
| cube | 144→149→144→143→145° | +5.0 / −4.2 / −0.9 / +1.8 | **60.1°** | 1.35–1.50 | 不可观测（正方形足迹→协方差退化） |

**结论：STEP7 的"PCA 不可行"在修正内参后仍然成立，但机制变了。**
不是（主要）"单视角透视畸变 / 多视角点云圆形化"，而是：
4 视角表面点在世界系取并集后，XY 足迹的各向异性本就弱（伸长比仅 1.15–1.5），
且残留各向异性中有一部分是**视角几何固定**的伪信号 —— cube 的 PCA 角在物体旋转 135° 期间
几乎恒定（144±5°）就是直接证据。

### 测量二：偏航到底值不值得做（`batch_test_final.py --rot`，新加能力）

用**现有硬编码偏航**，把三个物体整体旋转后再抓（`scripts/batch_test_final.py --rot N`）：

| 施加旋转 | 总计 | duck | teddy | cube |
|---------|------|------|-------|------|
| 0°（n=10） | **30/30 (100%)** | 10/10 | 10/10 | 10/10 |
| +45°（n=2） | **6/6 (100%)** | 2/2 | 2/2 | 2/2 |
| +90°（n=2） | **4/6 (67%)** | 2/2 | 2/2 | **0/2** |

**结论：偏航容差远比"最大接触面积"的理想化预期宽** —— 相对错位 45° 零代价，
连 90° 错位对 duck/teddy 也 2/2 成功。唯一失败是 cube 在 90° 下 0/2（每次只有 1 个接触点、
穿透 −0.002 的临界夹持），而 cube 本来就有临界夹持记录（历史上出现 2/4 接触点、Δz≈1cm 的滑脱），
**n=2 无法把这次失败与 cube 固有的临界性分开**。

### 附带发现（已修/待记录）

`stabilize_objects()` 会把每个物体的朝向重置为单位四元数，
因此 `MULTI_OBJECTS_CONFIG` 里的 `euler` 实际**被丢弃**（配置与实际朝向不一致）。
`batch_test_final.py --rot` / `diagnose_yaw.py` 因此在 stabilize 之后显式重新施加配置朝向。

### 待决策（三选一，见下方决策卡）

> ⚠️ **2026-09-13 订正（见 STEP14）**：本节"偏航容差 ±45° 零代价"的测量用的是**弱判据**（只看 Δz>1cm），
> 会把"被顶起后掉回"和"空夹"算成成功。判据升级后重测：**cube 角对角（yaw 偏离面对面 30–45°）0/10 空夹（夹持开度 0.0mm）**，
> 面对面 10/10。"偏航不是瓶颈"的结论据此**作废** —— 偏航是真实需求。

1. **不做**（承认当前 3 物体 + pick-and-lift 场景下偏航不是瓶颈）；
2. **做 A：仿真惯性张量 oracle**（`getDynamicsInfo` 主轴，0.5h，只在仿真有效，作上界/标签）；
3. **做 D：去掉 per-object 常量**（按点云足迹纵横比自适应决定抓取方式，1–2h，新物体无需改代码）。

触发重估的条件（建议写进文档）：出现**细长/扁平物体**（偏航直接决定能否夹住）、
需要**定向放置/插入**（不只是 pick-and-lift）、或换成**真机**（无 GT oracle）。

- 明细: `runs/yaw_diag_20260912_144655.json`、`runs/batch_stats_20260912_*.json`（rot=45/90）
- 日志: `D:\HermesWorkspace\{yaw_diag.log, yaw_value.log}`

---

## STEP14: 成功判据升级 + cube 失效根因（并订正 STEP13 的偏航结论）

**日期**: 2026-09-13 | **commit**: `65cfa81`（代码）+ 本文档

### 1. 判据升级（为什么必须做）

旧判据只有 `Δz > 1cm`，会把两类假成功算进去：
* **被顶起后掉回**：夹爪撞到物体把它顶起 >1cm，随后物体落回桌面 —— 旧判据在"顶起瞬间"读到的 Δz 已经达标；
* **空夹**：两指间什么都没有，但物体被机械臂带动/蹭起。

新判据（四条同时满足，`GRASP_LEGACY_CRITERION=1` 可回退）：

| # | 条件 | 对应的失败原因码 |
|---|------|-----------------|
| 1 | 轻提 Δz > 1cm | `dz`（压根没提起来）|
| 2 | 抬到高位**保持 240 仿真步**后 Δz 仍 > 1cm | `slipped`（提起后滑落）|
| 3 | 夹爪开度 > 5mm（两指间确实有物体）| `empty_grip`（空夹）|
| 4 | 保持结束时不再接触支撑面（桌面/地面）| `on_support`（仍压着桌面）|

`grasp_object()` 改为返回 `dict{ok, reason, dz_cm, dz_hold_cm, grip_mm, support_contacts}`，
batch harness 汇总"失败归因"与判据细节（夹持开度均值/保持Δz均值）。

### 2. 实测（n=10，严格判据）

| 配置 | 结果 | 夹持开度均值 | 备注 |
|------|------|-------------|------|
| 全物体 `rot=0` | **30/30 (100%)** | duck 67.4 / teddy 12.1 / cube 33.2 mm | 与旧判据的 30/30 一致 → **旧成功是真的** |
| cube `rot=0` | **10/10 (100%)** | 35.6 mm | |
| cube `rot=45` | **0/10 (0%)** | **0.0 mm** | 失败归因 `dz`×10 |
| cube `rot=90` | **0/10 (0%)** | **0.0 mm** | 失败归因 `slipped`×10（顶起后掉回）|

对照旧判据（同一批配置，早前测的）：`rot=45` 报 2/2、`rot=90` 报 2/2 —— **均为假象**。

### 3. cube 失效根因

**真因：cube 以"角对角"呈现时，平行夹爪闭合力会把方块挤出**（面对面对齐时 10/10、夹持开度 36mm；
角对角时 0/10、开度 0.0mm 空夹）。实测尺寸佐证：`dx=dy=0.050m`（面对面）vs `0.068m`（转 30°）。
接触日志里能看到先"2 contacts, dist=-0.018"（撞上）→ 随后空夹落地。

**混淆因素（值得记住的坑）**：`stabilize_objects()` 会把物体朝向重置为单位四元数，
所以 `rot=0` 时 cube 实际是 **0°（面对面）**（配置里的 `euler=[0,0,30]` 被丢弃），
而 `rot≠0` 时 `--rot` 的 re-apply 又把配置的 30° 恢复了 → 两个"旋转档位"比较的其实是
**面对面 vs 角对角**。若不做这个澄清，会误读成"90° 与 0° 几何等价却结果相反"。

### 4. 订正 STEP13

STEP13 的结论"偏航容差 ±45° 零代价 → 偏航不是瓶颈"**作废**：
它建立在弱判据 + cube 恰好面对面的双重巧合上。修正后的判断：
**对紧凑对称物体，偏航与夹爪轴的错位超过约 20° 就会失败**（0/10），偏航是真实需求。

### 5. 下一步（已排期）

* 用点云足迹的 **minAreaRect 角度**作为偏航（PCA 对正方形足迹退化，但最小面积矩形对它有定义），
  在 cube 0°/30°/45° 三档验证，目标把"角对角"从 0/10 提到 ≥9/10；
* 用严格判据**复测 ±2cm 抖动**与 duck/teddy 的旋转档位（旧数用弱判据测得）；
* 结果稳定后更新 README 顶部结果表。

- 证据: `runs/batch_stats_20260913_*.json`（A/B/C/D 四批）、`D:\HermesWorkspace\robustness_round.log`
- 对照日志: `D:\HermesWorkspace\yaw_value.log`（旧判据下的 rot=45/90）
